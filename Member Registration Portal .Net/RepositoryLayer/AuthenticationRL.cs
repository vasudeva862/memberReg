...
namespace RepositoryLayer
{
    public class AuthenticationRL : IAuthenticationRL
    {
        ...
        private async Task<string> CreateMemberID()
        {
            string MemberID = string.Empty;
            try
            {
                var Result = await _userDetails.Find(x => true).SortByDescending(x => x.MemberID).FirstOrDefaultAsync();
                if (Result == null)
                {
                    return "R-100";
                }

                int LastId = Convert.ToInt32(Result.MemberID.Split("-")[1]);
                LastId += 1;
